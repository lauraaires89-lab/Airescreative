...
- **Deploy — YA EN VIVO. Firebase Hosting es el único hosting activo (Vercel se dio de baja el 2026-09-01, a pedido de Laura, para no mantener dos copias del sitio).** Repo en GitHub: `github.com/lauraaires89-lab/Airescreative` (sigue siendo el control de versiones / fuente de verdad del código, solo que ya no dispara ningún deploy automático). Proyecto Firebase `Airescreative` (ID real: `airescreative-4a39e`). URL de respaldo: `https://airescreative-4a39e.web.app`. `firebase.json` sirve desde la raíz del repo (`"public": "."`) con un redirect 302 de `/` a `/site/index.html` (mismo motivo de siempre: el sitio vive en `site/*.html` con `assets/` en la raíz), e ignora `contenido-nuevo/`, archivos `.md`, `.git`, etc. `.firebaserc` fija el proyecto por defecto.
  ...
  - **Importante para futuros cambios de estructura de carpetas:** revisar que `firebase.json` siga apuntando bien — usa un **redirect** (cambia la URL del navegador a `/site/...`) y no un **rewrite** (mantiene la URL en `/`), porque con rewrite los links relativos internos (`trabajo.html`, `servicios.html`, etc.) se rompían con 404 al no resolver contra `/site/` (nos pasó primero con Vercel, mismo principio aplica acá).
