...
En `styleguide/index.html` y `index.html` (ambos self-contained, imágenes en base64):
...
2. **Home** (`index.html`) — **reconstruido completo el 2026-08-04** (Laura trajo una versión nueva desde otra carpeta/sesión, ejecutada a partir de `CODE_PROMPT_HOME_REBUILD.md`, y la fusionamos como base actual). Estructura actual, en orden:
...
- `index.html` — Home ✅
- `trabajo.html` — Proyectos (listado general) ✅
- `proyecto.html` — Plantilla de proyecto individual ✅ (una sola plantilla, sin páginas por proyecto todavía)
...
- **Deploy — YA EN VIVO. Firebase Hosting es el único hosting activo (Vercel se dio de baja el 2026-09-01, a pedido de Laura, para no mantener dos copias del sitio).** Repo en GitHub: `github.com/lauraaires89-lab/Airescreative` (sigue siendo el control de versiones / fuente de verdad del código, solo que ya no dispara ningún deploy automático). Proyecto Firebase `Airescreative` (ID real: `airescreative-4a39e`). URL de respaldo: `https://airescreative-4a39e.web.app`.
  - **Estructura del repo (reorganizada 2026-09-01):** los `.html` del sitio (`index.html`, `trabajo.html`, `servicios.html`, `sweetest.html`, `proyecto.html`) viven en la **raíz del repo**, junto con `assets/` — ya no están dentro de una carpeta `site/`. Se movieron ahí para que `airescreative.com` cargue directo sin necesitar ningún redirect (antes la URL se veía como `airescreative.com/site/index.html`, cosa que Laura no quería). `firebase.json` sirve todo con `"public": "."` sin redirects ni rewrites — ya no hace falta ninguno. Todas las rutas a `assets/` dentro de esos `.html` son relativas simples (`assets/...`, no `../assets/...`).
  - **El deploy es 100% manual:** después de cualquier cambio hay que (1) `git push` a GitHub como respaldo/historial, y (2) correr `firebase deploy --only hosting` desde la raíz del repo — ese segundo paso es el que realmente publica. Laura no tiene que hacer nada de esto, lo hace Claude en cada sesión.
...
- **Dominio propio conectado (2026-09-01).** Laura compró `airescreative.com` en **Namecheap** y lo conectó a Firebase Hosting. Configuración en Namecheap → Advanced DNS:
